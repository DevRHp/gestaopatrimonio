    ...
    if network:
        ...
        if is_super or check_password_hash(network.password, password):
            ...
            if is_super: 
                ...
                session['is_admin'] = True # Super admin is admin everywhere
...
@app.route('/super/networks', methods=['GET'])
def list_all_networks():
    ...
    results = []
    for n in nets:
        admin = User.query.get(n.admin_id)
        # Count users? Hard to count "auditors" as they correspond to just sessions. 
        # But we can count file owners associated with this network.
        file_count = FileMetadata.query.filter_by(network_id=n.id).count()
...
@app.route('/super/rename_network', methods=['POST'])
def rename_network_super():
    if not session.get('is_super_admin'): return jsonify({'error': 'Unauthorized'}), 403
    ...
    net = Network.query.get(net_id)
    if not net: return jsonify({'error': 'Network not found'}), 404
    
    # Check duplicate
    if Network.query.filter_by(name=new_name).first():
...
@app.route('/super/network/<int:net_id>/users', methods=['GET'])
def list_network_users(net_id):
    if not session.get('is_super_admin'): return jsonify({'error': 'Unauthorized'}), 403
    ...
    users = []
    # 1. The Admin
    admin = User.query.get(net.admin_id)
    if admin:
        users.append({
            'id': admin.id,
            'email': admin.email,
    ...
    # 2. Anyone who has uploaded files to this network (Implied access)
    # Get distinct user_ids from FileMetadata for this network
    contributor_ids = db.session.query(FileMetadata.user_id).filter_by(network_id=net_id).distinct().all()
    for (uid,) in contributor_ids:
        if uid and uid != net.admin_id:
    ...
    return jsonify({'users': users, 'network_name': net.name})

@app.route('/super/delete_user', methods=['POST'])
def super_delete_user():
    ...
    # Cascade delete?
    # Delete networks owned by this user? Or reassign? 
    # For now, let's just delete the user. Database integrity might need checking if foreign keys aren't cascading.
    # App config says: app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False. 
    # Models don't define cascade explicitly. 
    # Let's verify if we need to delete networks first.
    
    owned_networks = Network.query.filter_by(admin_id=u.id).all()
    for n in owned_networks:
        db.session.delete(n) # Delete network if admin is deleted
